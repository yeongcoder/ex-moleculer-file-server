...
const brokerConfig: BrokerOptions = {
	...
	created : (broker: ServiceBroker): void => {
		switch(process.env.NODE_ENV) {
			case "develop":
				require("dotenv").config({ path: require("path").join(__dirname, "env/local.env") });
				break;
			//case "production":
			default:
				require("dotenv").config({ path: require("path").join(__dirname, "env/prod.env") });
				break;
		}

		console.log("#####################################################");
		console.log("###	MOLECULER CREATED!");
		
		// NODE_ENV값은 일반적으로 사용되는 값으로 추가한 모듈에 사용될수 있음으로 
		// 성능에 영향을 줄수도 있다. 따라서 기본적으로 설정해 두는 것이 좋다.
